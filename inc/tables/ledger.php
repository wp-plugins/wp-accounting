...
class directory_ledger_table extends WP_List_Table {
    ...
    function column_default($item, $column_name){
		...
		if(!empty($this->sales_meta)){
			foreach($this->sales_meta as $v){
				if($column_name == $v->post_name){
					$value = wpa_moneyFormat($value,$item->currency);
		...
		switch($column_name){
			...
			case 'end_balance':
				$val = wpa_moneyFormat($value,$item->currency);
				return $val;
			break;
			case 'action':
				return '<a href="'.add_query_arg('delete',$item->ledger_id,$_SERVER['REQUEST_URI']).'" class="confirm button-secondary">Delete</a> <a href="'.add_query_arg('balancereset',$item->ledger_id,$_SERVER['REQUEST_URI']).'" class="confirm button-secondary">Reset Bal.</a>';
			break;
			case 'account':
				return $this->accounts[$item->account_id];
			break;
			case 'type':
				if($item->type_id == 1){
					...
					if($meta = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix . "wpaccounting_meta WHERE ledger_id='".$item->ledger_id."'")){
						foreach($meta as $m){
							if(isset($this->sales_meta_titles[$m->meta_key])){
								$value .= '<tr><td>'.$this->sales_meta_titles[$m->meta_key].'</td><td>'. wpa_moneyFormat($m->meta_value,$item->currency).'</td></tr>';
							}
						}
					}
					$value .= '</table>';
    ...
    function prepare_items() {
		...
		foreach($accounts as $v){
			$this->accounts[$v->ID] = $v->post_title;
		}
        
		/* -- Preparing your query -- */
		$query = "SELECT l.ledger_id, l.ledger_date, l.amount, l.type_id, l.details, l.end_balance, l.currency, l.account_id, IF(l.type_id = 0, 'Manual Adjustment' , IF(l.type_id = 1, 'Income', CONCAT('Expense: ',(SELECT post_title FROM ".$wpdb->prefix."posts WHERE ID=l.type_id)))) as type";
		...
		if(isset($_GET['filter_currency']) && $_GET['filter_currency'] != ''){
			$wheres[] = "l.currency='".esc_sql($_GET['filter_currency'])."'";
		}
		...
		if(!isset($_GET["orderby"]) || $_GET["orderby"] == '' || $_GET["orderby"] == 'ledger_date'){
			$orderby .= ' '.$order.', ledger_id '.$order;
			$order = '';
		}
		$query .= ' ORDER BY ';
	...
	function bulk_actions(){
		...
		echo '<option value="sale" '.($_GET['filter_type'] == 'sale' ? 'SELECTED' : '').'>Income</option>';
		...
		if(get_option('wpaccounting_multicurrency') == 1){
			...
			echo '<select name="filter_currency" onchange="this.form.submit();">';
			echo '<option value="">All Currencies</option>';
			foreach($currencies as $v){
				echo '<option value="'.$v->post_title.'" '.(isset($_GET['filter_currency']) && $_GET['filter_currency'] == $v->post_title ? 'SELECTED' : '').'>'.$v->post_title.'</option>';
			}
			echo '</select>';
		}
